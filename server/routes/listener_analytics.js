...
export async function handleListenerAnalyticsRoutes(req, res) {
  ...
  const match = url.pathname.match(/^\/analytics\/listener\/(\d+)\/summary/);
  if (!match) {
  ...
  try {
    ...
    const [[{ totalPlaylistsCreated = 0 }]] = await db.query(
      `SELECT COUNT(*) as totalPlaylistsCreated FROM Playlist WHERE ListenerID = ?`, [listenerId]
    );
    ...
    const [listens] = await db.query(`
      SELECT
        ...
        CASE WHEN LS.ListenerID IS NULL THEN 0 ELSE 1 END AS liked
      FROM Listen_History LH
      JOIN Song S ON LH.SongID = S.SongID
    ...
    res.end(JSON.stringify({
      totals: [
        { label: "Total Songs Played", value: totalPlayed },
        { label: "Total Likes", value: totalLikes },
        { label: "Total Playlists Created", value: totalPlaylistsCreated },
