...
export async function handleSubscriptionRoutes(req, res) {
  ...
  return;
    ...
    // GET subscription by ListenerID
...
if (pathname === "/subscriptions" && method === "POST") {
  ...
  req.on("end", async () => {
    try {
      ...
      const { ListenerID, DateStarted, DateEnded, IsActive } = data;

      if (!ListenerID || !DateStarted) {
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Missing required fields: ListenerID, DateStarted" }));
        return;
      }

      console.log("🧠 Inserting into DB:", [ListenerID, DateStarted, DateEnded || null, IsActive ?? 1]);

      try {
        const [result] = await db.query(
          "INSERT INTO Subscription (ListenerID, DateStarted, DateEnded, IsActive) VALUES (?, ?, ?, ?)",
          [ListenerID, DateStarted, DateEnded || null, IsActive ?? 1]
        );
...
if (pathname.startsWith("/subscriptions/") && method === "PUT") {
  ...
  req.on("end", async () => {
    try {
      const { ListenerID, DateStarted, DateEnded, IsActive } = JSON.parse(body);
