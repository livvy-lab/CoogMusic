// server/routes/artist_avatar.js
...
export function handleSetArtistAvatar(req, res, artistId) {
  ...
  res.setHeader("Access-Control-Allow-Origin", "*");
      ...
      const Prefix = process.env.S3_ARTIST_PREFIX || "images/artists";
      const Key = `${Prefix}/${artistId}/avatar_${randomUUID()}${ext}`;
      ...
      await db.query(
        "UPDATE Artist SET image_media_id = ?, PFP = ? WHERE ArtistID = ?",
        [mediaId, canonicalUrl, artistId]
      );
      ...
      console.error("[artist-avatar] upload error:", e);
